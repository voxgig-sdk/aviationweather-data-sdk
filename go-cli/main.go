// Command AviationweatherData-cli is a CLI + REPL for the AviationweatherData SDK,
...
const prompt = "aviationweather-data"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "air_sigmet airport cache cwa g_airmet metar pirep station_info taf tcf"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      AVIATIONWEATHER_DATA_APIKEY (api key), AVIATIONWEATHER_DATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: AVIATIONWEATHER_DATA_APIKEY carries the API key and
	// AVIATIONWEATHER_DATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("AVIATIONWEATHER_DATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("AVIATIONWEATHER_DATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAviationweatherDataSDK(opts)
