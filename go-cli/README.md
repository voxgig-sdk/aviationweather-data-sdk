...
AQL-driven command-line client **and** interactive REPL for the AviationweatherData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/aviationweather-data-cli)
make build

# 2. See usage (words, entities, env vars)
./aviationweather-data-cli --help
...
./aviationweather-data-cli list airport

# 5. Override the API base URL for a single call
AVIATIONWEATHER_DATA_BASE=https://api.example.com ./aviationweather-data-cli list air_sigmet

# 6. No arguments -> interactive REPL
./aviationweather-data-cli
aviationweather-data> list air_sigmet
aviationweather-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/aviationweather-data-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/aviationweather-data-cli list air_sigmet
   ```
...
Run with no arguments to open a REPL (prompt `aviationweather-data>`). Each line is
evaluated as its own AQL expression:

```text
$ ./aviationweather-data-cli
aviationweather-data> list air_sigmet
aviationweather-data> /help
aviationweather-data> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `air_sigmet`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/aviationweather-data-cli`. |
...
(`./aviationweather-data-cli <expr>`) and interactively (the REPL), and expressions compose the
