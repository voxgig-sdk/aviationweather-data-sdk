...
local MetarEntity = {}
MetarEntity.__index = MetarEntity


function MetarEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MetarEntity)
  self._name = "metar"
  self._client = client
...
function MetarEntity:get_name()
  return self._name
end


function MetarEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MetarEntity.new(self._client, opts)
end


function MetarEntity:data_set(args)
  if args ~= nil then
...
function MetarEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MetarEntity:match_set(args)
  if args ~= nil then
...
function MetarEntity:match_get()
...
function MetarEntity:stream(action, args, callopts)
...
function MetarEntity:_run_op(ctx, post_done)
  local utility = self._utility
