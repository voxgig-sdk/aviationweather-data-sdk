...
local AirSigmetEntity = {}
AirSigmetEntity.__index = AirSigmetEntity


function AirSigmetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AirSigmetEntity)
  self._name = "air_sigmet"
  self._client = client
...
function AirSigmetEntity:get_name()
  return self._name
end


function AirSigmetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AirSigmetEntity.new(self._client, opts)
end


function AirSigmetEntity:data_set(args)
  if args ~= nil then
...
function AirSigmetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AirSigmetEntity:match_set(args)
  if args ~= nil then
...
function AirSigmetEntity:match_get()
...
function AirSigmetEntity:stream(action, args, callopts)
...
function AirSigmetEntity:_run_op(ctx, post_done)
  local utility = self._utility
