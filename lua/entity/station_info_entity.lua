...
local StationInfoEntity = {}
StationInfoEntity.__index = StationInfoEntity


function StationInfoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StationInfoEntity)
  self._name = "station_info"
  self._client = client
...
function StationInfoEntity:get_name()
  return self._name
end


function StationInfoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StationInfoEntity.new(self._client, opts)
end


function StationInfoEntity:data_set(args)
  if args ~= nil then
...
function StationInfoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StationInfoEntity:match_set(args)
  if args ~= nil then
...
function StationInfoEntity:match_get()
...
function StationInfoEntity:stream(action, args, callopts)
...
function StationInfoEntity:_run_op(ctx, post_done)
  local utility = self._utility
