...
local TafEntity = {}
TafEntity.__index = TafEntity


function TafEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TafEntity)
  self._name = "taf"
  self._client = client
...
function TafEntity:get_name()
  return self._name
end


function TafEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TafEntity.new(self._client, opts)
end


function TafEntity:data_set(args)
  if args ~= nil then
...
function TafEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TafEntity:match_set(args)
  if args ~= nil then
...
function TafEntity:stream(action, args, callopts)
...
---@param reqmatch TafListMatch
---@param ctrl? table
---@return Taf[]
---@return string? err
function TafEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function TafEntity:_run_op(ctx, post_done)
  local utility = self._utility
