-- AviationweatherData SDK Cwa entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CwaEntity = {}
CwaEntity.__index = CwaEntity


function CwaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CwaEntity)
  self._name = "cwa"
  self._client = client
...
function CwaEntity:get_name()
  return self._name
end


function CwaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CwaEntity.new(self._client, opts)
end


function CwaEntity:data_set(args)
  if args ~= nil then
...
function CwaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CwaEntity:match_set(args)
  if args ~= nil then
...
function CwaEntity:stream(action, args, callopts)
...
---@param reqmatch CwaListMatch
---@param ctrl? table
---@return Cwa[]
---@return string? err
function CwaEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CwaEntity:_run_op(ctx, post_done)
  local utility = self._utility
