...
The Lua SDK for the AviationweatherData API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:AirSigmet(nil):list(nil, nil)
...
local result, err = client:AviationweatherData(nil):load(
  { id = "test01" }, nil
)
