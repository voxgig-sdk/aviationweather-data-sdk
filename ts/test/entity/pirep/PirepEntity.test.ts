...
describe('PirepEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'pirep.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set AVIATIONWEATHER_DATA_TEST_PIREP_ENTID JSON to run live')
      return
    }
    ...
    let pirep_ref01_data = Object.values(setup.data.existing.pirep)[0] as any

    // LIST
    const pirep_ref01_ent = client.Pirep()
    const pirep_ref01_match: any = {}

    const pirep_ref01_list = await pirep_ref01_ent.list(pirep_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/pirep/PirepTestData.json')
  ...
  const idmapEnvVal = process.env['AVIATIONWEATHER_DATA_TEST_PIREP_ENTID']
