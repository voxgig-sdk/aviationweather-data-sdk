...
describe('MetarDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new AviationweatherDataSDK({
      system: { fetch: async () => ({}) }
    })
  ...
  test('direct-list-metar', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-metar', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['AVIATIONWEATHERDATA_TEST_METAR_ENTID']
