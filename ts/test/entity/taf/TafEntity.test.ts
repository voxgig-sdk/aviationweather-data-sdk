...
describe('TafEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'taf.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set AVIATIONWEATHER_DATA_TEST_TAF_ENTID JSON to run live')
      return
    }
    ...
    let taf_ref01_data = Object.values(setup.data.existing.taf)[0] as any

    // LIST
    const taf_ref01_ent = client.Taf()
    const taf_ref01_match: any = {}

    const taf_ref01_list = await taf_ref01_ent.list(taf_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/taf/TafTestData.json')
  ...
  const idmapEnvVal = process.env['AVIATIONWEATHER_DATA_TEST_TAF_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'AVIATIONWEATHER_DATA_TEST_TAF_ENTID': idmap,
    'AVIATIONWEATHER_DATA_TEST_LIVE': 'FALSE',
    'AVIATIONWEATHER_DATA_TEST_EXPLAIN': 'FALSE',
    'AVIATIONWEATHER_DATA_APIKEY': 'NONE',
  })

  idmap = env['AVIATIONWEATHER_DATA_TEST_TAF_ENTID']
