...
describe('CwaEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'cwa.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set AVIATIONWEATHER_DATA_TEST_CWA_ENTID JSON to run live')
      return
    }
    ...
    let cwa_ref01_data = Object.values(setup.data.existing.cwa)[0] as any

    // LIST
    const cwa_ref01_ent = client.Cwa()
    const cwa_ref01_match: any = {}

    const cwa_ref01_list = await cwa_ref01_ent.list(cwa_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/cwa/CwaTestData.json')
  ...
  const idmapEnvVal = process.env['AVIATIONWEATHER_DATA_TEST_CWA_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'AVIATIONWEATHER_DATA_TEST_CWA_ENTID': idmap,
    'AVIATIONWEATHER_DATA_TEST_LIVE': 'FALSE',
    'AVIATIONWEATHER_DATA_TEST_EXPLAIN': 'FALSE',
    'AVIATIONWEATHER_DATA_APIKEY': 'NONE',
  })

  idmap = env['AVIATIONWEATHER_DATA_TEST_CWA_ENTID']
