...
module AviationweatherDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AviationweatherDataResponse.new({})
    ...
    if fetch_err
      ...
      response = AviationweatherDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AviationweatherDataResponse.new(fetched)
    else
