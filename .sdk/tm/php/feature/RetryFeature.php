...
// AviationweatherData SDK retry feature
...
class AviationweatherDataRetryFeature extends AviationweatherDataBaseFeature
{
    ...
    public function init(AviationweatherDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AviationweatherDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AviationweatherDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AviationweatherDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
