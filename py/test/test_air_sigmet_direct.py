...
class TestAirSigmetDirect:

    def test_should_direct_list_air_sigmet(self):
        setup = _air_sigmet_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-air_sigmet", "live" if setup["live"] else "unit")
        if _skip:
...
def _air_sigmet_direct_setup(mockres):
