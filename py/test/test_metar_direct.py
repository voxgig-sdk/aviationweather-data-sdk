...
class TestMetarDirect:

    def test_should_direct_list_metar(self):
        setup = _metar_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-metar", "live" if setup["live"] else "unit")
        if _skip:
...
def _metar_direct_setup(mockres):
