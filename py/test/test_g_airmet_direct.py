...
class TestGAirmetDirect:

    def test_should_direct_list_g_airmet(self):
        setup = _g_airmet_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-g_airmet", "live" if setup["live"] else "unit")
        if _skip:
...
def _g_airmet_direct_setup(mockres):
