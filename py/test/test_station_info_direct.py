...
class TestStationInfoDirect:

    def test_should_direct_list_station_info(self):
        setup = _station_info_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-station_info", "live" if setup["live"] else "unit")
        if _skip:
...
def _station_info_direct_setup(mockres):
