...
class TestPirepDirect:

    def test_should_direct_list_pirep(self):
        setup = _pirep_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-pirep", "live" if setup["live"] else "unit")
        if _skip:
...
def _pirep_direct_setup(mockres):
