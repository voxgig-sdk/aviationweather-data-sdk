...
class TestTafDirect:
    ...
    def test_should_direct_list_taf(self):
        setup = _taf_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-taf", "live" if setup["live"] else "unit")
        if _skip:
