...
The Python SDK for the AviationweatherData API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.AirSigmet(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.AviationweatherData(None).load(
    {"id": "test01"}, None
)
