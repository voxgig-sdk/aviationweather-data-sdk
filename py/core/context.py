...
from core.control import AviationweatherDataControl
...
class AviationweatherDataContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = AviationweatherDataControl()
        ...
        if isinstance(sp, AviationweatherDataSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, AviationweatherDataResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, AviationweatherDataResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return AviationweatherDataOperation({})
        ...
        op = AviationweatherDataOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return AviationweatherDataError(code, msg, self)
