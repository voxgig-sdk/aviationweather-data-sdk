# aviationweather-data-mcp

[MCP](https://modelcontextprotocol.io) server exposing the AviationweatherData SDK as
two agent tools — `aviationweather-data_list` and `aviationweather-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/aviationweather-data-mcp)
make build

# 2. Provide credentials via the environment
export AVIATIONWEATHER_DATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user aviationweather-data \
  -- /absolute/path/to/aviationweather-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./aviationweather-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// aviationweather-data_list: first page of records
{ "entity": "air_sigmet" }
{ "entity": "air_sigmet", "query": { } }

// aviationweather-data_load: one record by id
{ "entity": "cache", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/aviationweather-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user aviationweather-data \
     -- "$PWD"/dist/*/aviationweather-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `aviationweather-data_list` and `aviationweather-data_load` tools now appear
   in new sessions. Ask the agent to *"list air_sigmet using aviationweather-data"*
   and it calls `aviationweather-data_list` with `{"entity":"air_sigmet"}`.
...
```sh
export AVIATIONWEATHER_DATA_APIKEY=sk_live_xxx            # API key
export AVIATIONWEATHER_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `aviationweather-data_list` tool
...
### Call the `aviationweather-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `aviationweather-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `aviationweather-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AVIATIONWEATHER_DATA_APIKEY` | API key sent with every request. |
| `AVIATIONWEATHER_DATA_BASE` | Optional override of the API base URL. |
...
```sh
./aviationweather-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"aviationweather-data_load","arguments":{"entity":"cache","query":{"id":1}}}}'
```
