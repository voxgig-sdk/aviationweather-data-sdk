...
// AviationweatherData SDK feature test
//
...
class FtCtrl extends AviationweatherDataControl
{
...
class FtClient
{
    ...
    private AviationweatherDataUtility $utility;

    public function __construct(AviationweatherDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): AviationweatherDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AviationweatherDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AviationweatherDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AviationweatherDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AviationweatherDataConfig::make_feature($name);
    ...
    private static function buildUrl(AviationweatherDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AviationweatherDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AviationweatherDataResponse($fetched);
        }
    ...
    private function fail(AviationweatherDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AviationweatherDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
