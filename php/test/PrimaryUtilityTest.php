...
// AviationweatherData SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        AviationweatherDataSDK $client,
        AviationweatherDataUtility $utility
    ): AviationweatherDataContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new AviationweatherDataContext($ctxmap, null);
    ...
    private static function fixctx(AviationweatherDataContext $ctx, AviationweatherDataSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?AviationweatherDataError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new AviationweatherDataError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new AviationweatherDataResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new AviationweatherDataBaseFeature();
    ...
    {
        $hook_client = AviationweatherDataSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends AviationweatherDataBaseFeature {
            public $hook_fn = null;
            public function TestHook(AviationweatherDataContext $ctx): void
            {
    ...
    {
        $init_client = AviationweatherDataSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends AviationweatherDataBaseFeature {
            public $init_fn = null;
            ...
            public function init(AviationweatherDataContext $ctx, array $options): void
            {
    ...
    {
        $calls = [];
        $live_client = new AviationweatherDataSDK([
            'system' => [
    ...
    {
        $blocked_client = new AviationweatherDataSDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new AviationweatherDataSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = AviationweatherDataSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new AviationweatherDataOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = AviationweatherDataSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof AviationweatherDataError) ? $result->err->msg : (string)$result->err;
