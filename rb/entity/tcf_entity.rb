...
class TcfEntity
  ...
  def make
    opts = @_entopts.dup
    TcfEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AviationweatherDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Tcf, Hash] the current Tcf data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AviationweatherDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Tcf fields)
  def match_get
  ...
  # Load a single Tcf.
  #
  # @param reqmatch [TcfLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Tcf.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Tcf, Hash] the loaded Tcf; raises AviationweatherDataError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AviationweatherDataHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
