...
class StationInfoDirectTest < Minitest::Test
  def test_direct_list_station_info
    setup = station_info_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-station_info", setup[:live] ? "live" : "unit")
    if _should_skip
...
def station_info_direct_setup(mockres)
  ...
  if live
    ...
    client = AviationweatherDataSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
