...
class AirSigmetDirectTest < Minitest::Test
  def test_direct_list_air_sigmet
    setup = air_sigmet_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-air_sigmet", setup[:live] ? "live" : "unit")
    if _should_skip
...
def air_sigmet_direct_setup(mockres)
