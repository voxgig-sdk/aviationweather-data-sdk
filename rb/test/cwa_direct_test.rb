...
class CwaDirectTest < Minitest::Test
  def test_direct_list_cwa
    setup = cwa_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-cwa", setup[:live] ? "live" : "unit")
    if _should_skip
