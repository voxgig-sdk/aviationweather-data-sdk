...
class GAirmetDirectTest < Minitest::Test
  def test_direct_list_g_airmet
    setup = g_airmet_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-g_airmet", setup[:live] ? "live" : "unit")
    if _should_skip
...
def g_airmet_direct_setup(mockres)
