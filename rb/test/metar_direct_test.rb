...
class MetarDirectTest < Minitest::Test
  def test_direct_list_metar
    setup = metar_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-metar", setup[:live] ? "live" : "unit")
    if _should_skip
...
def metar_direct_setup(mockres)
